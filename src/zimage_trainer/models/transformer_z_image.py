...
class ZImageTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        # Support Tensor input (wrap in list for compatibility)
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            for layer in self.noise_refiner:
                x = torch.utils.checkpoint.checkpoint(
                    layer, x, x_attn_mask, x_freqs_cis, adaln_input, use_reentrant=False
                )
        else:
            for layer in self.noise_refiner:
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            for layer in self.context_refiner:
                cap_feats = torch.utils.checkpoint.checkpoint(
                    layer, cap_feats, cap_attn_mask, cap_freqs_cis, use_reentrant=False
                )
        else:
            for layer in self.context_refiner:
