...
def find_latest_checkpoint(output_dir: Path, output_name: str):
    ...
    # 解析检查点类型和步数/epoch
    # 格式: {output_name}_step{step}.safetensors 或 {output_name}_epoch{epoch}.safetensors
    ...
    # 查找对应的优化器文件
    # 格式: {output_name}_step{step}_optimizer.pt 或 {output_name}_epoch{epoch}_optimizer.pt
    ...
    if optimizer_path.exists():
        ...
        # 如果找不到对应的优化器文件，也返回 safetensors 路径（可能用户手动删除了 optimizer.pt）
...
def save_optimizer_state(optimizer, lr_scheduler, global_step, epoch, output_dir: Path, output_name: str, checkpoint_type: str, step_or_epoch: int, accelerator: Accelerator):
    ...
    Args:
        ...
        output_dir: 输出目录
        output_name: 输出文件名前缀
        checkpoint_type: 'step' 或 'epoch'
        step_or_epoch: 步数或 epoch 数
...
def load_optimizer_state(optimizer, lr_scheduler, optimizer_path: Path, accelerator: Accelerator, num_update_steps_per_epoch: int):
    ...
    Args:
        optimizer: 优化器
        lr_scheduler: 学习率调度器
        optimizer_path: 优化器状态文件路径
        accelerator: Accelerator 实例
        num_update_steps_per_epoch: 每个 epoch 的更新步数（用于计算 epoch）

    Returns:
        tuple: (global_step, start_epoch, skip_steps_in_epoch) 恢复的训练进度
    """
    ...
    # 计算在当前 epoch 中需要跳过的步数
    skip_steps_in_epoch = global_step % num_update_steps_per_epoch

    logger.info(f"  [OK] 已加载优化器状态: {optimizer_path}")
    logger.info(f"  [RESUME] 恢复训练进度: global_step={global_step}, saved_epoch={saved_epoch}, start_epoch={start_epoch}, skip_steps={skip_steps_in_epoch}")
...
def main():
    ...
    # num_update_steps_per_epoch 已在第591行计算，这里直接使用
    ...
    if latest_safetensors is not None:
        ...
        if latest_optimizer is not None and latest_optimizer.exists():
            ...
            # 加载 LoRA 权重
            logger.info(f"[LOAD] 加载 LoRA 权重: {latest_safetensors}")
            network.load_weights(latest_safetensors, device=accelerator.device)
            ...
            logger.warning(f"[WARN] 将从头开始训练（模型权重已加载）")
            # 如果只有模型文件没有优化器文件，可以选择加载模型权重但从头训练
            # 这里我们选择不自动加载，让用户决定
            # network.load_weights(latest_safetensors, device=accelerator.device)
    ...
    progress_bar = tqdm(total=args.max_train_steps, initial=global_step, desc="Training", disable=True)
