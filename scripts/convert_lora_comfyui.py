...
def convert_to_comfyui(input_path: str, output_path: str, dtype: str = None):
    ...
    for key, value in state_dict.items():
        ...
        for qkv_type in ["to_q", "to_k", "to_v"]:
            ...
            if actual_marker:
                ...
                # e.g., "layers.0.attention.to_q.lora_down.weight" 或 "layers.0.attention.to_q.lora_A"
                # -> base = "layers.0.attention", part = "lora_down" 或 "lora_A"
                base_path = clean_key.split(actual_marker)[0] + ".attention"
                suffix = clean_key.split(actual_marker)[1]  # "lora_down.weight", "lora_A", "alpha" 等

                # 支持两种格式：
                # 1. lora_down.weight / lora_up.weight (标准格式)
                # 2. lora_A / lora_B (用户格式)
                if suffix.endswith(".weight"):
                    lora_part = suffix.replace(".weight", "")  # "lora_down" or "lora_up"
                else:
                    lora_part = suffix  # "alpha" 或其他

                if lora_part == "lora_A":
                    lora_part = "lora_down"
                elif lora_part == "lora_B":
                    lora_part = "lora_up"
    ...
    for key, value in other_tensors.items():
        ...
        elif ".lora_A" in comfy_key:
            # 用户格式: lora_A -> lora_down
            base = comfy_key.replace(".lora_A.weight", "")
            new_key = to_comfyui_lora_key(base, "lora_down")
        elif ".lora_B" in comfy_key:
            # 用户格式: lora_B -> lora_up
            base = comfy_key.replace(".lora_B.weight", "")
            new_key = to_comfyui_lora_key(base, "lora_up")
